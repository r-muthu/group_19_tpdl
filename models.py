...
# ResNet34 + BiLSTM + Attention Model
class ResNet34BiLSTMAttentionDropout(nn.Module):
    def __init__(self, classes=10, lstm_hidden=128, lstm_layers=2, attention_size=256, dropout_prob = 0.2):
        super(ResNet34BiLSTMAttentionDropout, self).__init__()
        ...
        # BiLSTM Layer
        self.lstm = nn.LSTM(input_size=512, hidden_size=lstm_hidden, num_layers=lstm_layers, batch_first=True, bidirectional=True)
    ...
    def forward(self, x):
        ...
        # print(f"ResNet Output Shape: {x.shape}")  # Debugging the shape    
        x = x.squeeze(2).permute(0, 2, 1)   # [B, 7, 512]
        ...
        # Pass through LSTM
        lstm_out, _ = self.lstm(x)  # Output shape: [batch_size, seq_len, hidden*2]
